...
async function getList(req: AuthenticatedRequest, res: VercelResponse): Promise<void> {
  const contacts = await prisma.subordinateContact.findMany({
...
async function createContact(req: AuthenticatedRequest, res: VercelResponse): Promise<void> {
  ...
  if (existing) {
    return res.status(409).json({
      error: 'Conflict',
      message: `Subordinate contact with name '${name}' already exists`,
    });
  }

  const contact = await prisma.subordinateContact.create({
    data: {
      name,
